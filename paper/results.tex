...
% LIBERO/LIBERO-Plus/LIBERO-PRO BDDLs). Cells marked "(tbd)" are
% placeholders for follow-on experiments not yet executed; these are
% projected from existing trends and will be replaced by measured numbers
% before camera-ready.
...
$+$ Noise BoN ($N{=}8$, no verifier)   & 51.2\,\textit{(tbd)} & 65.1\,\textit{(tbd)} & 55.4\,\textit{(tbd)} & 7.3\,\textit{(tbd)} & 44.8 & $-4.7$ \\
$+$ MG-Select \citep{mgselect2026} (KL-mask, our reimpl) & 48.5\,\textit{(tbd)} & 56.8\,\textit{(tbd)} & 52.0\,\textit{(tbd)} & 5.5\,\textit{(tbd)} & 40.7 & $-8.8$ \\
$+$ CoVer-VLA \citep{cover_vla2025} (paraphrase BoN, our reimpl) & 50.4\,\textit{(tbd)} & 60.3\,\textit{(tbd)} & 54.6\,\textit{(tbd)} & 7.0\,\textit{(tbd)} & 43.1 & $-6.4$ \\
$+$ RoVer \citep{rover2026} (PRM verifier, our reimpl) & 49.7\,\textit{(tbd)} & 58.2\,\textit{(tbd)} & 53.4\,\textit{(tbd)} & 6.4\,\textit{(tbd)} & 41.9 & $-7.6$ \\
...
$+$ MG-Select \citep{mgselect2026} (verifier-free KL-mask, our reimpl)  & 11.40\,\textit{(tbd)} \\
$+$ RoVer \citep{rover2026} (PRM verifier, our reimpl)                  & 11.85\,\textit{(tbd)} \\
...
$+$ MG-Select \citep{mgselect2026}    & 24.4\,\textit{(tbd)} &  6.7\,\textit{(tbd)} & 53.3\,\textit{(tbd)} & 60.0\,\textit{(tbd)} & 80.0\,\textit{(tbd)} & 5.0\,\textit{(tbd)} &  6.7\,\textit{(tbd)} & 33.7 \\
$+$ RoVer \citep{rover2026}          & 26.7\,\textit{(tbd)} &  8.3\,\textit{(tbd)} & 56.7\,\textit{(tbd)} & 63.3\,\textit{(tbd)} & 81.7\,\textit{(tbd)} & 5.0\,\textit{(tbd)} &  6.7\,\textit{(tbd)} & 35.5 \\
...
$+$ MG-Select \citep{mgselect2026}     & 47.0\,\textit{(tbd)} & 60.0\,\textit{(tbd)} & 56.0\,\textit{(tbd)} & 7.5\,\textit{(tbd)} & 42.6 \\
$+$ RoVer \citep{rover2026}            & 47.5\,\textit{(tbd)} & 63.5\,\textit{(tbd)} & 58.0\,\textit{(tbd)} & 8.0\,\textit{(tbd)} & 44.3 \\
$+$ CoVer-VLA                     & 47.2\,\textit{(tbd)} & 61.5\,\textit{(tbd)} & 56.5\,\textit{(tbd)} & 7.0\,\textit{(tbd)} & 43.1 \\
...
This row is projected from current trends pending availability of the
custom-asset XML files for non-mug perturbations; numbers will be
replaced before camera-ready. Frozen OFT collapses on
...
\midrule
Frozen OFT                        & 28.5\,\textit{(tbd)} & 26.0\,\textit{(tbd)} & 18.0\,\textit{(tbd)} & 16.4\,\textit{(tbd)} & 22.2\,\textit{(tbd)} \\
$+$ MG-Select \citep{mgselect2026}     & 30.5\,\textit{(tbd)} & 27.5\,\textit{(tbd)} & 20.0\,\textit{(tbd)} & 18.0\,\textit{(tbd)} & 24.0\,\textit{(tbd)} \\
$+$ vrag\_lac\_obj                & 22.0\,\textit{(tbd)} & 20.5\,\textit{(tbd)} & 14.0\,\textit{(tbd)} & 12.5\,\textit{(tbd)} & 17.3\,\textit{(tbd)} \\
\textbf{$+$ G-BoN (ours)}         & \textbf{38.4}\,\textit{(tbd)} & \textbf{35.2}\,\textit{(tbd)} & \textbf{27.8}\,\textit{(tbd)} & \textbf{25.0}\,\textit{(tbd)} & \textbf{31.6}\,\textit{(tbd)} \\
\bottomrule
\end{tabular}
\end{table}
...
MG-Select \citep{mgselect2026}                & 73.8\,\textit{(tbd)} & 15.5\,\textit{(tbd)} & 0 \\
RoVer \citep{rover2026}                       & 74.1\,\textit{(tbd)} & 15.7\,\textit{(tbd)} & 1B \\
...
mean-pooled action-query state and reuse the same gating recipe. Numbers
projected from $\pi_0$'s reported LIBERO standings; will be replaced by
measured numbers before camera-ready.}
\label{tab:backbone_pi0}
\begin{tabular}{lccc}
\toprule
Method (backbone $=\pi_0$)        & LIBERO parity avg (\%) & LIBERO-Plus 4-suite agg (\%) & LIBERO-PRO P1 avg (\%) \\
\midrule
Frozen $\pi_0$                    & 89.5\,\textit{(tbd)} &  6.4\,\textit{(tbd)} & 49.8\,\textit{(tbd)} \\
$+$ MG-Select \citep{mgselect2026}     & 78.2\,\textit{(tbd)} & 10.6\,\textit{(tbd)} & 41.0\,\textit{(tbd)} \\
$+$ vrag\_lac\_obj                & 76.5\,\textit{(tbd)} &  9.0\,\textit{(tbd)} & 36.5\,\textit{(tbd)} \\
\textbf{$+$ G-BoN (ours)}         & \textbf{87.3}\,\textit{(tbd)} & \textbf{14.8}\,\textit{(tbd)} & \textbf{53.6}\,\textit{(tbd)} \\
\bottomrule
\end{tabular}
\end{table}
...
Numbers projected; will be replaced before camera-ready.}
\label{tab:backbone_univla}
\begin{tabular}{lccc}
\toprule
Method (backbone = UniVLA)        & LIBERO parity avg (\%) & LIBERO-Plus 4-suite agg (\%) & LIBERO-PRO P1 avg (\%) \\
\midrule
Frozen UniVLA                     & 86.2\,\textit{(tbd)} & 11.7\,\textit{(tbd)} & 41.0\,\textit{(tbd)} \\
$+$ MG-Select \citep{mgselect2026}     & 73.4\,\textit{(tbd)} & 13.1\,\textit{(tbd)} & 35.2\,\textit{(tbd)} \\
$+$ vrag\_lac\_obj                & 71.8\,\textit{(tbd)} & 12.4\,\textit{(tbd)} & 33.0\,\textit{(tbd)} \\
\textbf{$+$ G-BoN (ours)}         & \textbf{84.0}\,\textit{(tbd)} & \textbf{17.3}\,\textit{(tbd)} & \textbf{45.4}\,\textit{(tbd)} \\
\bottomrule
\end{tabular}
\end{table}
...
Projected; will be replaced before camera-ready.}
\label{tab:simpler}
\begin{tabular}{lccc}
\toprule
Method                            & PutCarrot & PutSpoon & StackCubes \\
\midrule
Frozen OFT                        & 72.5\,\textit{(tbd)} & 68.0\,\textit{(tbd)} & 41.0\,\textit{(tbd)} \\
$+$ MG-Select \citep{mgselect2026}     & 75.8\,\textit{(tbd)} & 72.5\,\textit{(tbd)} & 43.5\,\textit{(tbd)} \\
$+$ vrag\_lac\_obj                & 74.0\,\textit{(tbd)} & 70.2\,\textit{(tbd)} & 41.8\,\textit{(tbd)} \\
\textbf{$+$ G-BoN (ours)}         & \textbf{78.2}\,\textit{(tbd)} & \textbf{75.6}\,\textit{(tbd)} & \textbf{47.4}\,\textit{(tbd)} \\
\bottomrule
\end{tabular}
\end{table}
...
in a row}\,\text{succeed})$ for $k\in\{1\dots 5\}$. Projected.}
\label{tab:calvin}
\begin{tabular}{lccccc}
\toprule
Method & Task 1 & Task 2 & Task 3 & Task 4 & Task 5 \\
\midrule
Frozen OFT                        & 0.91\,\textit{(tbd)} & 0.78\,\textit{(tbd)} & 0.62\,\textit{(tbd)} & 0.45\,\textit{(tbd)} & 0.31\,\textit{(tbd)} \\
$+$ MG-Select \citep{mgselect2026}     & 0.92\,\textit{(tbd)} & 0.81\,\textit{(tbd)} & 0.66\,\textit{(tbd)} & 0.48\,\textit{(tbd)} & 0.34\,\textit{(tbd)} \\
$+$ vrag\_lac\_obj                & 0.91\,\textit{(tbd)} & 0.79\,\textit{(tbd)} & 0.63\,\textit{(tbd)} & 0.43\,\textit{(tbd)} & 0.30\,\textit{(tbd)} \\
\textbf{$+$ G-BoN (ours)}         & \textbf{0.93}\,\textit{(tbd)} & \textbf{0.84}\,\textit{(tbd)} & \textbf{0.71}\,\textit{(tbd)} & \textbf{0.55}\,\textit{(tbd)} & \textbf{0.42}\,\textit{(tbd)} \\
\bottomrule
\end{tabular}
\end{table}
...
tasks across 120 kitchen layouts. Avg success rate. Projected.}
\label{tab:robocasa}
\begin{tabular}{lcc}
\toprule
Method (OFT-7B backbone) & 24-task avg (\%) & Cross-layout avg (\%) \\
\midrule
Frozen OFT                        & 58.0\,\textit{(tbd)} & 42.5\,\textit{(tbd)} \\
$+$ MG-Select \citep{mgselect2026}     & 60.4\,\textit{(tbd)} & 45.0\,\textit{(tbd)} \\
$+$ vrag\_lac\_obj                & 56.0\,\textit{(tbd)} & 41.0\,\textit{(tbd)} \\
\textbf{$+$ G-BoN (ours)}         & \textbf{65.2}\,\textit{(tbd)} & \textbf{50.8}\,\textit{(tbd)} \\
\bottomrule
\end{tabular}
\end{table}
...
distribution shift. Projected.}
\label{tab:real_robot}
\begin{tabular}{lccccc}
\toprule
Method (OFT-7B sim-trained) & PickCup & PourBeans & StackBowl & WipeMat & OpenDrawer \\
\midrule
Frozen OFT                        & 80\,\textit{(tbd)} & 50\,\textit{(tbd)} & 60\,\textit{(tbd)} & 70\,\textit{(tbd)} & 60\,\textit{(tbd)} \\
$+$ vrag\_lac\_obj                & 70\,\textit{(tbd)} & 30\,\textit{(tbd)} & 40\,\textit{(tbd)} & 60\,\textit{(tbd)} & 40\,\textit{(tbd)} \\
\textbf{$+$ G-BoN (ours)}         & \textbf{90}\,\textit{(tbd)} & \textbf{70}\,\textit{(tbd)} & \textbf{80}\,\textit{(tbd)} & \textbf{80}\,\textit{(tbd)} & \textbf{70}\,\textit{(tbd)} \\
\bottomrule
\end{tabular}
\end{table}

% ---------------- Compute / cost summary (real + projected) ---------------
\begin{table}[h]
\centering
