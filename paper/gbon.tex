...
\item \textbf{LIBERO-PRO P1 task-distractor.} G-BoN beats
      ...
      perturbations) shows $+8$~pp average gain
      (Table~\ref{tab:lpro_p2}).
...
\paragraph{Cross-backbone and cross-benchmark transfer.}
...
and SIMPLER/CALVIN/RoboCasa/real-robot.
...
$\pi_0$~\citep{pi02024} and UniVLA are reported in
Tables~\ref{tab:backbone_pi0} and~\ref{tab:backbone_univla}.
...
(\_with\_mug); P2 position-perturbation split (Table~\ref{tab:lpro_p2}).
(iv) SIMPLER~\citep{simpler2024} sim-to-real bench;
...
the strongest re-implemented baselines (MG-Select at
$-8.8$~pp and RoVer at $-7.6$~pp), G-BoN's $-4.4$~pp is the
...
noise BoN ($5.36\%$), MG-Select ($11.40\%$),
RoVer ($11.85\%$), CoVer-VLA ($10.36\%$), and retrieval
...
\paragraph{Cross-backbone generality.}
...
\paragraph{Cross-benchmark generality.}
SIMPLER (Table~\ref{tab:simpler}), CALVIN
(Table~\ref{tab:calvin}), RoboCasa (Table~\ref{tab:robocasa}), and
real-robot transfer (Table~\ref{tab:real_robot}) show G-BoN extending
the qualitative pattern: best-of-class plug-in test-time gain,
...
\paragraph{Cross-backbone experiments.}
Headline experiments use OpenVLA-OFT-7B.
Cross-backbone results on $\pi_0$ and UniVLA
(Tables~\ref{tab:backbone_pi0}, \ref{tab:backbone_univla}) confirm
that the plug-in nature of G-BoN (no backbone retraining) transfers
cleanly across VLA families.

\paragraph{Real-robot transfer.}
Real-robot transfer (Table~\ref{tab:real_robot}) uses a Franka FR3 with
a sim-to-real wrapper.
The gating mechanism transfers naturally because $z_t$ is a
property of the policy alone, so a real-robot OOD distribution shift
